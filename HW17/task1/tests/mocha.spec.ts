...
import {StringChanges} from "../StringChanges.js";
import {TDeleteExcludeTestData, TGetSubstrTestData, TGetSymbolTestData, TStringReverseTestData} from "../TestDataTypes.js";
...
describe ("Check StringChanges class methods", () => {
    describe("Check getSymbol method", () => {
        ...
        getSymbolTestData.forEach((value, index) => {
            it(`test data : ${value.testData} ; expected result : ${value.expectedResult}`, () => {
                const resultValue : string = StringChanges.getSymbol(value.testData[0], value.testData[1]);
                expect(resultValue).to.equal(value.expectedResult);
            })

            it(`type of ${value.expectedResult} is string`, () => {
                const typeOfValue : string = typeof StringChanges.getSymbol(value.testData[0], value.testData[1]);
                expect(typeOfValue).to.equal("string");
            })
        })
    })
    describe("Check getSubstr method", () => {
        ...
        testData.forEach(value => {
            it(`test data : ${value.testData} ; expected result : ${value.expectedResult}`, () => {
                ...
                expect(resultValue).to.deep.equal(value.expectedResult);
            })
        })
    })
